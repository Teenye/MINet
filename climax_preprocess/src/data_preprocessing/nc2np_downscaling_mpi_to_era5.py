...
def nc2np(path_mpi, path_era5, variables, years, save_dir, partition, num_shards_per_year):
    ...
    for year in tqdm(years):
        ...
        for var in variables:
            ...
            if len(ds_mpi[mpi_code].shape) == 3:  # surface level variables
                ds_mpi[mpi_code] = ds_mpi[mpi_code].expand_dims("val", axis=1)
                ds_era5[code] = ds_era5[code].expand_dims("val", axis=1)
                
                data_mpi = ds_mpi[mpi_code].to_numpy()[:HOURS_PER_YEAR]
                data_era5 = ds_era5[code].to_numpy()[:HOURS_PER_YEAR]
                
                reshaped_data = data_mpi.reshape(-1, data_mpi.shape[-2], data_mpi.shape[-1])
                resized_data = np.array([resize(frame, (32,64),order=1) for frame in reshaped_data])
                data_mpi = resized_data.reshape(data_mpi.shape[0], data_mpi.shape[1], 32, 64)
                ...
                assert len(ds_mpi[mpi_code].shape) == 4
                ...
                for level in all_levels:
                    ...
                    data_mpi = ds_level_mpi[mpi_code].to_numpy()[:HOURS_PER_YEAR]
                    reshaped_data = data_mpi.reshape(-1, data_mpi.shape[-2], data_mpi.shape[-1])
                    resized_data = np.array([resize(frame, (32,64),order=1) for frame in reshaped_data])
                    data_mpi = resized_data.reshape(data_mpi.shape[0], data_mpi.shape[1], 32, 64)
                        
                    # remove the last 24 hours if this year has 366 days
                    np_mpi_vars[f"{var}_{level}"] = data_mpi
...
@click.command()
@click.option("--path_mpi", type=str, default='/data1/tengyue/data/mpi/5.625deg_algined')
@click.option("--path_era5", type=str, default='/data1/tengyue/data/era5/1.40625deg_algined')
@click.option("--save_dir", type=str, default='/data1/tengyue/data/new32_downscale_mpi_5.625_era5_1.40625')
@click.option(
    "--variables",
    "-v",
    type=click.STRING,
